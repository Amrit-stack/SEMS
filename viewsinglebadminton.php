...
if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM `badmintonsingle` WHERE CONCAT(`crn`,`playername`, `batch`,`faculty`,`position`) LIKE '%".$valueToSearch."%'";
  ...
  {
 $query = "SELECT * FROM badmintonsingle";
 $search_result = filterTable($query);
}
...
<html>
<head>
  <title> SINGLE BADMINTON PLAYERS</title>
</head>
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    ...
    <form class="form-inline my-2 my-lg-0" action="viewsinglebadminton.php" method="POST">
...
<td><a href='deletesinglebadminton.php?crn={$row["crn"]}' class='btn btn-danger'>Delete</a> </td>
<td><a href='addwinner.php?crn=$row[crn]&name=$row[playername]&fl=$row[faculty]&bt=$row[batch]&ps=$row[position]' class='btn btn-danger'>Make Winner</a> </td>
</tr>
";
?>
</tr>
...
function fetch_data()  
 {  
      $output = '';  
      $connect = mysqli_connect("localhost", "root", "", "sports");  
      $sql = "SELECT *FROM badmintonsingle";  
...
$obj_pdf->SetTitle("SINGLE BADMINTON PLAYERS LIST");  
...
$content = '';  
$content .= '  
<h4 align="center">SINGLE BADMINTON PLAYERS</h4><br /> 
